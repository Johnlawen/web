...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'notify-marketing') {
      const { eventName, notifyAll, notifyUnbooked } = req.body;
      
      if (!process.env.RESEND_API_KEY) {
        return res.status(500).json({ error: 'Manca la chiave API di Resend nel server.' });
      }

      // Consolidate Audience (using a Map to deduplicate by lowercase email)
      const audienceMap = new Map();

      // 1. Get all newsletter subscribers
      if (data.subscribers) {
        data.subscribers.forEach(s => {
          if (s.email) audienceMap.set(s.email.toLowerCase(), s.name || 'Utente');
        });
      }
      
      // 2. Get all past attendees
      if (data.pastOrders) {
        data.pastOrders.forEach(o => {
          if (o.email) audienceMap.set(o.email.toLowerCase(), o.name || 'Utente');
        });
      }
      
      // 3. Get all active orders (just in case they aren't in subscribers)
      if (data.orders) {
        data.orders.forEach(o => {
          if (o.email) audienceMap.set(o.email.toLowerCase(), o.name || 'Utente');
        });
      ...
      // If they want to notify ONLY unbooked, remove anyone currently booked for this event
      if (notifyUnbooked && !notifyAll) {
        if (data.orders) {
          data.orders.forEach(o => {
            if (o.email && (o.event || '').toLowerCase() === (eventName || '').toLowerCase()) {
              audienceMap.delete(o.email.toLowerCase());
            }
          });
        }
      }

      const finalAudience = Array.from(audienceMap.entries()).map(([email, name]) => ({ email, name }));

      if (finalAudience.length === 0) {
        return res.status(200).json({ success: true, audienceCount: 0, message: 'Nessun contatto trovato.' });
      }
      ...
      try {
        ...
        // Max 50 per batch via Resend Bcc/batching. We will just fire them individually here since it's a small list, 
        // but wrapped in Promise.all to send concurrently (up to a limit if huge, but fine for small events).
        // Actually, Resend supports passing an array of strings to `to` up to 50 items. 
        // Let's send in batches of 50 using `bcc` so they don't see each other.
        const BATCH_SIZE = 50;
        const emailsToSend = finalAudience.map(u => u.email);
        ...
        for (let i = 0; i < emailsToSend.length; i += BATCH_SIZE) {
          ...
          await resend.emails.send({
            ...
            to: 'noreply@luccagroove.com', // Primary to self
            bcc: batch,                    // BCC to audience
            subject: \`🔥 Nuovo Evento: \${eventName} - Biglietti Disponibili!\`,
            html: \`
              <div style="font-family:sans-serif;max-width:600px;margin:0 auto;background:#000;color:#fff;padding:20px;border-radius:8px;text-align:center;">
                <h1 style="color:#FF6B00;letter-spacing:0.1em;font-size:2rem;margin-bottom:4px;">LUCCA GROOVE</h1>
                <p style="color:#888;font-size:0.8rem;letter-spacing:0.2em;margin-bottom:24px;">LUCCA, ITALIA</p>
                
                <div style="background:#111;padding:24px;border-radius:12px;border:1px solid #333;margin-bottom:24px;">
                  <h2 style="margin-top:0;color:#fff;font-size:1.5rem;">Un nuovo evento ti aspetta!</h2>
                  <h3 style="color:#FF6B00;font-size:1.8rem;margin:10px 0;">\${eventName}</h3>
                  <p style="color:#eee;line-height:1.6;font-size:1.1rem;">
                    Siamo pronti per una nuova serata indimenticabile. I biglietti sono ora disponibili. 
                    Assicurati il tuo posto prima che vadano sold out!
                  </p>
                  
                  <a href="https://luccagroove.com/" style="display:inline-block;margin-top:20px;background:#FF6B00;color:#000;padding:12px 24px;text-decoration:none;font-weight:bold;border-radius:6px;font-size:1.1rem;">
                    PRENOTA ORA
                  </a>
                </div>
                
                <p style="color:#555;font-size:0.8rem;">
                  Hai ricevuto questa email perché sei iscritto alla newsletter o hai partecipato a eventi precedenti di Lucca Groove.
                </p>
              </div>
            \`
          });
        ...
        return res.status(200).json({ success: true, audienceCount: emailsToSend.length });
      } catch (error) {
        console.error('Errore invio email massivo:', error);
        return res.status(500).json({ error: 'Errore durante l\\'invio con Resend.' });
      }
    }
